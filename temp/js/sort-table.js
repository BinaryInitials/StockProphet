var compare = {                           // Declare compare object
  marketcap: function(a, b) {
	var c = a.replace(/T/,'E12').replace(/B/,'E9').replace(/M/,'E6').replace(/K/,'E3')
	var d = b.replace(/T/,'E12').replace(/B/,'E9').replace(/M/,'E6').replace(/K/,'E3')
	if(parseFloat(c) < parseFloat(d)){
		return -1;
	}else {
		return parseFloat(c) > parseFloat(d) ? 1 : 0;
	}
  },
  
  number: function(a, b) {
	var c = a.replace(/N[^A-Za-z0-9]A/,'-1').replace(/[+,%]/,'');
	var d = b.replace(/N[^A-Za-z0-9]A/,'-1').replace(/[+,%]/,'');
	if (parseFloat(c.replace(/,/,'')) < parseFloat(d.replace(/,/,''))) {
  		return -1;
  	} else {
  		return parseFloat(c.replace(/,/,'')) > parseFloat(d.replace(/,/,'')) ? 1 : 0;
  	}
  },
